...
function calculateScores(unit, req) {
  ...
  if (req.minBudget || req.maxBudget) {
    ...
    if (unit.price >= min && unit.price <= max) {
      ...
      // Under budget is okay but less preferred
      scores.budget = 0.8;
    } else {
      // Over budget penalty
...
function getHighlights(unit, req, scores) {
  ...
  if (scores.budget === 1.0 && req.maxBudget) {
    ...
    if (savings > 0) {
      highlights.push(`${formatCurrency(savings)} under budget`);
    }
  }
  ...
  if (scores.location === 1.0) {
    highlights.push('Preferred location');
  }

  if (unit.view) {
    highlights.push(`${capitalize(unit.view)} view`);
  }
